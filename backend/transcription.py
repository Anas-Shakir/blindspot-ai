...
def transcribe(audio_url: str, lecture_id: int) -> list[TranscriptSegment]:
    ...
    if not source.exists():
        raise FileNotFoundError(f"Audio/video file not found: {local_path}")
    if not source.is_file():
        raise ValueError(f"Input is not a file: {local_path}")
    if source.stat().st_size == 0:
        raise ValueError(f"Input file is empty: {local_path}")
